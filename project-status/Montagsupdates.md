 ...
 * [Die `agency_id` ist in `routes.txt` optional](https://developers.google.com/transit/gtfs/reference#routestxt), wenn nur eine Agency vorhanden ist. Das kam in den vorherigen Datenquellen nicht vor, hatte aber neuerdings Abstürze erzeugt.
 * Eine unserer Tabellen hatte recht knapp bemessene Spaltenbreiten für `trip_id`, `route_id` und `stop_id`, die von einer neuen Datenquelle teilweise um Faktor 4 übertroffen wurden.
 * In unserem Stammgebiet VBN, genauer genommen in unserer Heimatstadt Brauschweig, haben wir neuerdings fast alle Fahrten doppelt. Da sind wir noch immer auf Fehlersuche.
 * Somit gibt es auch noch einiges zu tun:
   * Wir brauchen eine neue Startseite, die auf die verschiedenen Instanzen des Monitors verweist. Bisher ist nur unsere [VBN-Instanz unter https://monitor.dystonse.org](https://monitor.dystonse.org) verlinkt. 
 ...
 * Bisher musste eine neue Dystonse-Instanz einige Tage oder Wochen laufen, bis genug Echtzeitdaten gesammelt sind, um daraus nützliche Prognosen abzuleiten. Um diese Hürde etwas abzumildern, haben wir einen kleinen Workaround geschaffen:
   * Aus unseren umfangreichen Daten aus dem VBN haben wir eine `default_curves.exp` exportiert, also ein Verspätungsmodell, das nicht linienspezifisch ist, sondern nach allgemeinen Aspekten wie Verkehrsmittel, Tageszeit, etc. aufgeteilt ist.
   * Wir haben [Anweisungen](https://github.com/dystonse/dystonse-docker#set-up-default-statistics-for-quick-start) geschrieben, wie diese Daten einzubinden sind. Der Vorgang ist nämlich (noch?) nicht automatisiert. Siehe [Ticket #14 in dystonse-gtfs-data](https://github.com/dystonse/dystonse-gtfs-data/issues/14).
   * Außerdem etwas [neuer Code](https://github.com/dystonse/dystonse-gtfs-data/commit/e6106a7a026455b435f3dd8047157bffe9519ed1), der diese `default_curves.exp` mit der lokal erstellten `all_curves.exp` kombiniert, falls beides vorhanden ist, und [Logausgaben](https://github.com/dystonse/dystonse-gtfs-data/commit/bbfba80976f1c3e5269d0f6c706f191b2e24f9ec) um diesen Vorgang sichtbar zu machen.
 * Wir haben neue Datenquellen erhalten! Und damit konnten wir mit wenig Aufwand weitere Instanzen von Dystonse in Betrieb nehmen:
   * __Berlin: https://monitor-vbb.dystonse.org/__ - [hier](https://v0.berlin-gtfs-rt.transport.rest/) betreibt Jannis einen [Konverter von Hafas nach GTFS-RT](https://github.com/derhuerst/berlin-gtfs-rt-server)
   * __Hamburg: https://monitor-hvv.dystonse.org/__ - und auch [hier](https://v0.hamburg-gtfs-rt.transport.rest/) betreibt Jannis einen [Konverter von Hafas nach GTFS-RT](https://github.com/derhuerst/hamburg-gtfs-rt-server)
   * __Rhein-Neckar: https://monitor-vrn.dystonse.org/__ - der VRN bietet schon lange Echtzeitdaten an, die wir auch schon seit dem Frühjahr 2020 sammeln. Nun haben wir endlich auch ein Monitor-Frontend dafür aktiviert. Dabei war uns nicht aufgefallen, dass der VRN leider vor einiger Zeit den Zugang zu den offenen Daten beschränkt hat. Nun ist eine [Anmeldung](https://www.vrn.de/opendata/) und manuelle Freischaltung nötig (die wir inzwischen auch haben), aber die Downloads sind nun nicht mehr automatisierbar und schlagen aktuell fehl. Daher ist die Instanz noch leer.
 ...
 * Dass wir nun mehrere Instanzen betreiben, macht ein paar Anpassungen nötig. Als erstes haben wir nun [ermöglicht](https://github.com/dystonse/dystonse-gtfs-data/commit/eedd465041cb8c26459cbb84d716219868379fa4), dass der Name des Verkehrsverbundes konfiguriert werden kann. Als nächstes braucht es unter anderem ordentliche Angaben der Datenquellen und eine bessere Unterscheidbarkeit der Instanzen.

## Längere Pause

_Fast zwei Monate waren vergangen, seitdem wir zuletzt Zeit und Muße für Dystonse gefunden haben, und fast drei Monate seitdem wir zuletzt direkt etwas am Code oder unserer Server-Konfiguration gemacht gemacht hatten. Am 11. und 12. Dezember fand das [XMAS Open Mobility Data Community Remote Meetup 2020](https://gobeta.de/veranstaltungen/xmas-open-mobility-data-community-remote-meetup-2020-xomdcrm20/) statt und hat uns neue Motivation, Input und Verbindungen gebracht. Davor hatten wir noch versucht, an unserem wöchentlichen Update-Rhythmus festzuhalten._
